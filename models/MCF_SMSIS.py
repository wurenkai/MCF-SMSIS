...
class Context_Geometry_Fusion32(SubModule): #IFGF 1/32
    def __init__(self, cv_chan, im_chan):
        super(Context_Geometry_Fusion32, self).__init__()

        self.semantic = nn.Sequential(
            BasicConv(im_chan, im_chan//2, kernel_size=1, stride=1, padding=0),
            nn.Conv2d(im_chan//2, cv_chan, 1))

        self.sw_conv = BasicConv(im_chan+192, im_chan, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, cv, feat,sq32):
        '''
        '''
        if sq32 != None:
            feat = torch.cat((feat, sq32), dim=1)
            feat = self.sw_conv(feat)
...
class Context_Geometry_Fusion16(SubModule):  #IFGF 1/16
    def __init__(self, cv_chan, im_chan):
        super(Context_Geometry_Fusion16, self).__init__()

        self.semantic = nn.Sequential(
            BasicConv(im_chan, im_chan//2, kernel_size=1, stride=1, padding=0),
            nn.Conv2d(im_chan//2, cv_chan, 1))

        self.sw_conv = BasicConv(im_chan + 64, im_chan, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, cv, feat,sq16):
        '''
        '''
        if sq16 != None:
            feat = torch.cat((feat, sq16), dim=1)
            feat = self.sw_conv(feat)
        feat = self.semantic(feat).unsqueeze(2)
        att = self.att(feat+cv)
        cv = torch.sigmoid(att)*feat + cv
...
class Context_Geometry_Fusion8(SubModule):  #IFGF 1/8
    def __init__(self, cv_chan, im_chan):
        super(Context_Geometry_Fusion8, self).__init__()

        self.semantic = nn.Sequential(
            BasicConv(im_chan, im_chan//2, kernel_size=1, stride=1, padding=0),
            nn.Conv2d(im_chan//2, cv_chan, 1))

        self.sw_conv = BasicConv(im_chan + 48, im_chan, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, cv, feat,sq8):
        '''
        '''
        if sq8 != None:
            feat = torch.cat((feat, sq8), dim=1)
            feat = self.sw_conv(feat)
        feat = self.semantic(feat).unsqueeze(2)
        att = self.att(feat+cv)
        cv = torch.sigmoid(att)*feat + cv
...
class hourglass_fusion(nn.Module):
    def __init__(self, in_channels):
        ...
        self.CGF_32 = Context_Geometry_Fusion32(in_channels*6, 160)  #IFGF 1/32
        self.CGF_16 = Context_Geometry_Fusion16(in_channels*4, 192)  #IFGF 1/16
        self.CGF_8 = Context_Geometry_Fusion8(in_channels*2, 64)  #IFGF 1/8

    def forward(self, x, imgs, sq32, sq16, sq8):
        conv1 = self.conv1(x)
        conv2 = self.conv2(conv1)
        conv3 = self.conv3(conv2)

        conv3 = self.CGF_32(conv3, imgs[4],sq32)  #IFGF 1/32
        conv3_up = self.conv3_up(conv3)

        conv2 = torch.cat((conv3_up, conv2), dim=1)
        conv2 = self.agg_0(conv2)

        conv2 = self.CGF_16(conv2, imgs[3], sq16)  #IFGF 1/16
        conv2_up = self.conv2_up(conv2)

        conv1 = torch.cat((conv2_up, conv1), dim=1)
        conv1 = self.agg_1(conv1)

        conv1 = self.CGF_8(conv1, imgs[2], sq8)  #IFGF 1/8
        conv = self.conv1_up(conv1)

        return conv

class MCF_Stereo_feature_ex(nn.Module):
    def __init__(self):
        super(MCF_Stereo_feature_ex, self).__init__()
    ...
    def forward(self, left,right):
        ...
        return features_left, features_right, stem_2x, stem_4x, stem_2y, stem_4y





class MCF_Stereo_depth(nn.Module):
    def __init__(self, maxdisp = 192):
        super(MCF_Stereo_depth, self).__init__()
        self.maxdisp = maxdisp
        self.spx = nn.Sequential(nn.ConvTranspose2d(2*32, 9, kernel_size=4, stride=2, padding=1),)
        self.spx_2 = Conv2x(32, 32, True)
        self.spx_4 = nn.Sequential(
            BasicConv(128, 32, kernel_size=3, stride=1, padding=1),
            nn.Conv2d(32, 32, 3, 1, 1, bias=False),
            nn.BatchNorm2d(32), nn.ReLU()
            )

        self.conv = BasicConv(128, 48, kernel_size=3, padding=1, stride=1)
    ...
    def forward(self, features_left, features_right,stem_2x,stem_4x,stem_2y,stem_4y, sq32 = None, sq16= None, sq8= None):
        features_left[1] = torch.cat((features_left[1], stem_4x), 1)
        features_right[1] = torch.cat((features_right[1], stem_4y), 1)

        features_left[0] = F.gelu(F.max_pool2d(features_left[0], 2, 2))
        features_right[0] = F.gelu(F.max_pool2d(features_right[0], 2, 2))

        features_left[0] = torch.cat((features_left[1], features_left[0]), dim=1)
        features_right[0] = torch.cat((features_right[1], features_right[0]), dim=1)
        ...
        cost = self.hourglass_fusion(volume, features_left, sq32, sq16, sq8)
...
class UNetHead(nn.Module):
    def __init__(self, n_classes = 1):
        super().__init__()
        feature_channels = [32, 48, 64, 192, 160]
        ...
        self.sq32 = SqueezeAttentionBlock(feature_channels[3], feature_channels[3])
        self.sq16 = SqueezeAttentionBlock(feature_channels[2],feature_channels[2])
        self.sq8 = SqueezeAttentionBlock(feature_channels[1], feature_channels[1])
    ...
    def forward(self, feature_scales):
        x = self.decode_1(feature_scales[4], feature_scales[3])
        sq32 = F.gelu(F.max_pool2d(x,2,2))
        x = self.decode_2(x, feature_scales[2])
        sq16 =  F.gelu(F.max_pool2d(x,2,2))
        x = self.decode_3(x, feature_scales[1])
        sq8 =  F.gelu(F.max_pool2d(x,2,2))
        x = self.decode_4(x, feature_scales[0])
        x = self.upsample(x)
        x = self.out_conv(x)
        return torch.sigmoid(x), sq32, sq16, sq8
