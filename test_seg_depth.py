...
#fe_state_dict = torch.load(args.loadckpt_fe, map_location=torch.device('cpu'))
...
for i in trange(len(test_limg)):
    ...
    with torch.no_grad():
        ...
        features_lefts, features_rights, stem_2xs, stem_4xs, stem_2ys, stem_4ys = fes_model(limg_tensor, rimg_tensor)
