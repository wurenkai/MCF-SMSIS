...
def save_imgs(img, msk, msk_pred, i, save_path, datasets, threshold=0.5, test_data_name=None):
    ...
    if datasets == 'retinal':
        ...
        #msk = np.where(np.squeeze(msk.cpu().detach().numpy(), axis=0) > 0.5, 1, 0)
    ...
    print(msk.shape)
    plt.subplot(3,1,2)
    plt.imshow(msk[:, :, 0], extent=[0, 160, 0, 128])
    plt.axis('off')
    #plt.subplot(3,1,2)
...
def save_imgs_disp(img, msk, msk_pred, i, save_path, datasets, threshold=0.5, test_data_name=None):
    img = img.squeeze(0).permute(1,2,0).detach().cpu().numpy()
    #print(img)
    img = img / 255. if img.max() > 1.1 else img
